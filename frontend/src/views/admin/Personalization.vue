      ...
      <div class="p-6 space-y-6">
        ...
        <div v-if="settings.wallpaperType === 'color'">
          ...
          <div class="flex items-center gap-3">
            ...
            <input
              ...
              class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
            />
          </div>
        </div>
        ...
        <div v-else>
          ...
          <div v-if="settings.wallpaper" class="mb-4">
            <div class="relative w-full h-40 rounded-lg overflow-hidden border-2 border-dashed border-slate-300 dark:border-slate-600">
              ...
              <div class="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                <button @click="updateSetting('wallpaper', '')" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">
                  移除图片
                </button>
              </div>
            </div>
          ...
          <div class="flex gap-3">
            <button
              @click="openGallery"
              class="flex-1 px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center gap-2"
            >
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            @input="updateSetting('mobileItemsPerRow', Number(($event.target as HTMLInputElement).value))"
            class="w-full"
          />
        ...
        <div>
          ...
          <input
            ...
            @input="updateSetting('tabletItemsPerRow', Number(($event.target as HTMLInputElement).value))"
            class="w-full"
          />
        ...
        <div>
          ...
          <input
            ...
            @input="updateSetting('desktopItemsPerRow', Number(($event.target as HTMLInputElement).value))"
            class="w-full"
          />
      ...
      <div class="p-6 space-y-6">
        <div>
          ...
          <textarea
            ...
            class="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white font-mono text-sm"
          />
        </div>
        <div>
    ...
    <div v-if="showGalleryModal" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl w-full max-w-4xl max-h-[80vh] overflow-hidden">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
          ...
          <button @click="showGalleryModal = false" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(80vh-80px)]">
          ...
          <div v-else-if="galleryImages.length === 0" class="text-center py-12 text-slate-500">
            暂无图片，请先上传图片到图库
          </div>
...
const customCSS = ref(settings.value.customCSS || '')
const customJS = ref(settings.value.customJS || '')
