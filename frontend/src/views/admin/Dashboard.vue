<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500">分组数量</p>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500">网站数量</p>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500">图片数量</p>
            <p class="text-2xl font-bold text-slate-800 dark:text-slate-200">{{ isLoading ? '加载中...' : stats.images }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-slate-500">用户数量</p>
            <p class="text-2xl font-bold text-slate-800 dark:text-slate-200">{{ isLoading ? '加载中...' : stats.users }}</p>
          </div>
    ...
    <!-- 快捷操作 -->
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
      <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">快捷操作</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">最近添加的网站</h3>
        <div class="space-y-3">
          <div 
          ...
          <div v-if="recentItems.length === 0" class="text-center py-8 text-slate-500">
            暂无数据
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">最近添加的分组</h3>
        <div class="space-y-3">
          <div 
          ...
          >
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-slate-500">{{ getItemsCount(group.id) }} 个网站</p>
            </div>
          </div>
          <div v-if="recentGroups.length === 0" class="text-center py-8 text-slate-500">
            暂无数据
          </div>
        </div>
      </div>
...
const quickActions = [
  { path: '/admin/groups', label: '添加分组', icon: 'ph:folder-plus-bold', bgClass: 'bg-orange-100 dark:bg-orange-900/30', iconClass: 'text-orange-500' },
  { path: '/admin/groups', label: '添加网站', icon: 'ph:globe-bold', bgClass: 'bg-blue-100 dark:bg-blue-900/30', iconClass: 'text-blue-500' },
  { path: '/admin/gallery', label: '上传图片', icon: 'ph:image-bold', bgClass: 'bg-green-100 dark:bg-green-900/30', iconClass: 'text-green-500' },
  { path: '/admin/personalization', label: '更换主题', icon: 'ph:paint-brush-bold', bgClass: 'bg-purple-100 dark:bg-purple-900/30', iconClass: 'text-purple-500' },
  { path: '/admin/export-import', label: '导出数据', icon: 'ph:export-bold', bgClass: 'bg-cyan-100 dark:bg-cyan-900/30', iconClass: 'text-cyan-500' },
  { path: '/admin/api', label: '查看API', icon: 'ph:code-bold', bgClass: 'bg-pink-100 dark:bg-pink-900/30', iconClass: 'text-pink-500' },
]
